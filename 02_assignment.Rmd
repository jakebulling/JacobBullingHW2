...
https://github.com/jakebulling/JacobBullingHW2.git
...
lending_recipe <- recipe(Class~., 
                         ...
                         step_rm(sub_grade, acc_now_delinq, delinq_amnt)%>%
...
```{r}
#Setting up tuning and penalty grid. 
...
# finalize workflow
lending_lasso_final_wf <- lending_wf %>% 
...
```{r}
rf_penalty_grid <- grid_regular(finalize(mtry(), lending_training %>% select(-Class)), min_n(), levels = 3)
...
forest_final_wf <- forest_workflow %>%
  finalize_workflow(best_param_2)

forest_fit <- forest_final_wf %>% 
...
lasso_explain <- 
  explain_tidymodels(
    ...
    y = lending_training %>%mutate(Class_numeric = as.numeric(Class == "good"))%>% pull(Class_numeric), 
    label = "lasso"
  )
...
box_plot <- 
  plot(lasso_mod_perf, rf_mod_perf, 
       geom = "boxplot")
hist_plot + box_plot
```


I would say the most interesting thing is that one of the residuals is a very tight normal distribution and the other is not tight at all. 
...
plot(forest_var_imp, show_boxplots = TRUE)
```


Interest rate is clearly the most important variable in the model. 
...
```{r}
cp_profile <-function(explain, obs, name) {
  predict_profile(explainer = explain, new_observation = obs, variables = c(name))

}

obs4 <- lending_training %>%
  slice(4)
income_profile <-cp_profile(rf_explain, obs4, "annual_inc")

income_profile %>%  filter(`_vname_` %in% c("annual_inc")) %>% 
...
```{r}
rf_pdp <- model_profile(explainer = rf_explain, variables = c("annual_inc", "int_rate"))
...
lending_final_fit <- lending_testing %>%
...
lending_final_fit %>%
  roc_curve(truth = Class, estimate = .pred_bad) %>%
  autoplot()
